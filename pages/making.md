...
3. Enter the name of the bucket exactly as the URL to host the site; I entered `resume.ramiyer.me`. I'll refer to it as `your.bucket.name` going forward.
4. Select the region and select **Next**.
5. In the screen that appears, click on **Next**, provided you don't want logging or versioning configured.
...
6. Search for **AmazonS3FullAccess** and proceed to review.
7. Click **Create user**.
...
5. In the BitBucket repository, create a new file at the root, called `wercker.yml`.
    ...
    deploy:
        steps:
            - s3sync:
                ...
                opts: --acl-public
    ```
7. Back in the Wercker screen, you should have a link that says something along the lines of, 'I already have a wercker.yml. Proceed with build.' Go ahead and click that to start the pipeline. Ensure the build part completes successfully. The code would not be deployed to S3 as yet.
8. You should see a tab on the screen, called **Environment**. Click on it.
9. Add the Amazon Access key ID and the Secret key here. Ensure to make both these **protected**. The key names should _exactly_ match the variable names in the `deploy` part of `wercker.yml` (without the leading `$`). Also, note the `s3://` before the bucket name. That's important, too.
    ```
...
1. CloudFront makes the content delivery fast. Granted, this is not an issue for small static sites, which brings us to the next point.
2. You can have an SSL certificate, issued by Amazon for free, tagged to your site to encrypt the connection to the site.

Let's first proceed with getting ourselves an SSL certificate. This is optional. I like to encrypt connections to my sites, so I chose it.

1. Open the [AWS Certificate Manager](https://console.aws.amazon.com/acm/) console.
2. Click on **Request a certificate**.
3. Enter the domain (or subdomain) name, which should be the URL where you want to host the site, as well as the name of the bucket, `your.bucket.name`. The prerequisite here is that you need to have sufficient authority on your domain, including receiving admin emails. An email would be sent to the admin address of the domain for verification.
